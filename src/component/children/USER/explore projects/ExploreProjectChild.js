...
export default function ExploreProjectChild() {
    ...
    const [postsByIdPrj, setPostsByIdPrj] = useState([])
    useEffect(() => {
        window.scrollTo(0, 0)
    })
    const handlePrev = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/exploreProject?id_project=${myParam}`)
            .then(res => {
        ...
        axios.get(`http://localhost:3001/getPostsByIdProject?idProject=${myParam}`)
            .then(res => {
                setPostsByIdPrj(res.data.postsByIdProject)
                console.log(res.data.postsByIdProject)
            })
                ...
                <div className={"grip"}>
                    <div className={`${style.container}`}>
                        ...
                        <div className={style.posts}>
                            {
                                postsByIdPrj?.map((data, index) => (
                                    <div className={style.post} key={index}>
                                        <img alt={"imgPost"}
                                             onClick={() => navigate(`/diy/${data.id_of_project}/${data.title_post}`, {state: {idPost: data.id_post,
